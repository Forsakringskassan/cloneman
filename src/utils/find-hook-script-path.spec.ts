...
describe("should return script path", () => {
    ...
    it.each(extensions)("%s", (extension) => {
        ...
        const result = findHookScriptPath("foo", "/path/to/hooks");
...
it("should return undefined if no hook exists", () => {
    expect.assertions(1);
    vol.fromJSON({});
    const result = findHookScriptPath("foo", "/path/to/hooks");
...
it("should return undefined if no script with matching name exists", () => {
    expect.assertions(1);
    vol.fromJSON({
