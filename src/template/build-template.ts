...
export async function buildTemplate(options: {
    ...
    for (const partiallyManagedFile of partiallyManagedFiles) {
        ...
        const partiallyManagedFilePath = path.join(
            filesDir,
            getStoredFileName(partiallyManagedFile.name),
        );
        const content = await fs.readFile(partiallyManagedFilePath, "utf8");
        const contentWithMarkers = await patchPartiallyManagedFile(
            partiallyManagedFile,
            content,
        );
        await fs.writeFile(
            partiallyManagedFilePath,
            contentWithMarkers,
            "utf8",
        );
    }
